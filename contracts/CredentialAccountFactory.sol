...
contract CredentialAccountFactory is BaseAccountFactory {
    ...
    constructor(
        ...
        BaseAccountFactory(
            address(new CredentialAccount(_entrypoint, address(this))),
            address(_entrypoint)
        )
    {}
    ...
    function onRegistered(string calldata username) external {
        ...
        require(
            ...
            "CredentialAccountFactory: not an account."
        );
        require(
            accountOfUsername[username] == address(0),
            "CredentialAccountFactory: username already registered"
        );
